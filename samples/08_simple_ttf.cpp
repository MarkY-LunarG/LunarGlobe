...
//
// File:                    samples/08_simple_ttf.cpp
...
class SimpleTtfApp : public GlobeApp {
   ...
   private:
    ...
    GlobeFont *_font;
    uint8_t _current_fps_font_index;
    uint8_t _next_fps_font_index;
    float _diff_ring_buffer[50];
    uint8_t _ring_buffer_index;
};

SimpleTtfApp::SimpleTtfApp() {
    ...
    _texture_2 = nullptr;
    _font = nullptr;
    _current_fps_font_index = 0;
    _next_fps_font_index = 0;
    _ring_buffer_index = 0;
}

SimpleTtfApp::~SimpleTtfApp() { Cleanup(); }

void SimpleTtfApp::Cleanup() {
    GlobeApp::PreCleanup();
...
bool SimpleTtfApp::Setup() {
    ...
    if (!_is_minimized) {
        ...
        if (nullptr == _font) {
            _font = _globe_resource_mgr->LoadFontMap("OpenSans-Regular.ttf", vk_setup_command_buffer, 32);
            if (nullptr == _font) {
                logger.LogError("Failed loading font");
                return false;
            }
        }
        ...
        if (!_font->LoadIntoRenderPass(_vk_render_pass, _width, _height)) {
...
bool SimpleTtfApp::Update(float diff_ms) {
    ...
    _diff_ring_buffer[_ring_buffer_index++] = diff_ms;
    ...
    uint32_t int_fps = static_cast<uint32_t>(1000.f / average_diff);
    if (int_fps > 99999) {
        int_fps = 99999;
    } else if (int_fps < 0) {
        int_fps = 0;
    }
    std::string fps_message = "FPS: ";
    std::stringstream stream;
    stream << std::setw(5) << std::setfill(' ') << int_fps;
    fps_message += stream.str();

    // NOTE: Render 1 font vertex buffer ahead so the data has time to reach it before
    // we encounter the draw for it.
    _current_fps_font_index = _next_fps_font_index;
    _next_fps_font_index = (_current_fps_font_index + 1) % _swapchain_count;
    if (!_font->UpdateStringText(1 + _next_fps_font_index, fps_message)) {
        GlobeLogger &logger = GlobeLogger::getInstance();
        logger.LogError("Failed updating FPS font string");
        return false;
    }
...
bool SimpleTtfApp::Draw() {
    ...
    _font->DrawString(vk_render_command_buffer, mvp, 0);
    _font->DrawString(vk_render_command_buffer, mvp, 1 + _current_fps_font_index);
...
static SimpleTtfApp *g_app = nullptr;

GLOBE_APP_MAIN() {
    GlobeInitStruct init_struct = {};
    GLOBE_APP_MAIN_BEGIN(init_struct)
    init_struct.app_name = "Globe App - Simple TTF App";
    ...
    // NOTE: Use Mailbox mode here so we get a higher FPS.  It's more GPU work, but
    // it will mean non-vsync limited performance.
    init_struct.present_mode = VK_PRESENT_MODE_MAILBOX_KHR;
