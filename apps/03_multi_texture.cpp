...
bool MultiTexApp::Setup() {
    ...
    if (!_is_minimized) {
        ...
        _texture_2 = _globe_resource_mgr->LoadTexture("cks_memorial_taipei_lake.jpg", vk_setup_command_buffer);
        ...
        GlobeShader *multi_tex_shader = _globe_resource_mgr->LoadShader("position_multi_texture");
        if (nullptr == multi_tex_shader) {
            logger.LogFatalError("Failed to load position_multi_texture shaders");
            return false;
        }
